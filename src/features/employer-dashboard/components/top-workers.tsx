...
/** The people who keep coming back — the partner's team, rated by the partner. */
    ...
    <Card>
      ...
      <div className="px-5 pb-5 pt-3">
        {isPending ? (
          ...
          <ErrorState compact title="Couldn't load your team" error={error} onRetry={() => void refetch()} retrying={isRefetching} />
        ) : rows.length === 0 ? (
          <EmptyState compact icon={Star} title="No ratings yet" description="Once you approve and rate completed shifts, the people you work with most appear here." />
        ) : (
          <ul className="flex flex-col divide-y divide-border">
            {rows.map(({ worker, shifts, avgRating }, i) => worker ? (
              <motion.li key={worker.id} {...stagger(i)} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
