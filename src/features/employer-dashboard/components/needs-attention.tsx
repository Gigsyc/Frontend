...
import { AlertTriangle, CheckCircle2, ChevronRight, ClipboardCheck, CreditCard, Star, UserPlus, type LucideIcon } from "lucide-react";
    ...
    <section aria-labelledby="needs-attention">
      ...
      {items.length === 0 ? (
        <p className="flex items-center gap-2.5 rounded-lg bg-surface px-5 py-4 text-sm text-fg-muted shadow-card">
          ...
          Nothing needs you right now. Applications, check-ins and invoices show up here when they need a decision.
        </p>
      ) : (
        <ul className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {items.map((item, i) => {
            ...
            return (
              <motion.li key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: EASE, delay: Math.min(i, 7) * 0.03 }}>
                <Link
                  href={item.href}
