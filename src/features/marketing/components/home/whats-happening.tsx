 ...
 * back to the product. Renders nothing when there is nothing on.
 */
export function WhatsHappening() {
  const { events, isPending, isError } = useUpcomingEvents(4);
  ...
  return (
    <Section tone="canvas" className="border-y border-border">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Link
          href="/events"
          className="inline-flex items-center gap-1.5 text-sm font-medium text-navy-700 underline-offset-4 hover:underline [&_svg]:size-4"
        >
